...
export async function POST(req: NextRequest) {
  try {
    ...
    // Parse the ref: image-{id}-{dimensions}-{format}
    const [, assetId, , format] = ref.split('-')
    const imageUrl = `https://cdn.sanity.io/images/${projectId}/${dataset}/${assetId}.${format}?w=1200&q=85`
      ...
      messages: [
        {
          ...
          content: [
            {
              type: 'image',
              source: { type: 'base64', media_type: contentType as 'image/jpeg' | 'image/png' | 'image/gif' | 'image/webp', data: imageBase64 },
            },
            {
              type: 'text',
...
Start directly with the subject — no preamble.`,
    ...
    if (!caption) {
      return NextResponse.json({ ok: false, error: 'Empty caption from Claude' }, { status: 500 })
    }
    ...
    console.log(`✅ Caption generated for ${payload._id}: "${caption.slice(0, 60)}…"`)
