  ...
  while (offset < buf.length - 4) {
    ...
    if (marker === 0xe1 && offset + 10 <= buf.length) {
      ...
      if (header === "Exif\x00\x00") {
        ...
        const le =
          buf.slice(tiffStart, tiffStart + 2).toString("ascii") === "II";
        const r16 = (o: number) =>
          le
            ? buf.readUInt16LE(tiffStart + o)
            : buf.readUInt16BE(tiffStart + o);
        const r32 = (o: number) =>
          le
            ? buf.readUInt32LE(tiffStart + o)
            : buf.readUInt32BE(tiffStart + o);
        console.log(
          `  Byte order: ${le ? "little-endian (II)" : "big-endian (MM)"}`,
        );
        ...
        function dumpIfd(ifdOff: number, label: string) {
          ...
          for (let i = 0; i < n; i++) {
            ...
            const type = r16(e + 2);
            const cnt = r32(e + 4);
            const rawValOffset = le
              ? buf.readUInt32LE(tiffStart + e + 8)
              : buf.readUInt32BE(tiffStart + e + 8);
            const name = TAG_NAMES[tag] ?? `tag_${tag}`;
            const typeName = TYPE_NAMES[type] ?? `type_${type}`;
            const sz =
              (
                { 1: 1, 2: 1, 3: 2, 4: 4, 5: 8, 7: 1, 9: 4, 10: 8 } as Record<
                  number,
                  number
                >
              )[type] ?? 1;
            ...
            console.log(
              `    [${tag}] ${name}  type=${typeName}(${type}) count=${cnt} ${inline ? `inline=0x${rawValOffset.toString(16)}` : `ptr=0x${rawValOffset.toString(16)}`}`,
            );
          }
        }
        ...
        const exifPtr = r32(
          ifd0Off +
            ...
            (() => {
              const n = r16(ifd0Off);
              for (let i = 0; i < n; i++) {
                if (r16(ifd0Off + 2 + i * 12) === 34665) return i * 12;
              }
              return -1000;
            })() +
            8,
...
function parseTiff(buf: Buffer, tiffStart: number): ExifData {
  ...
  const rs32 = (o: number) =>
    le ? buf.readInt32LE(tiffStart + o) : buf.readInt32BE(tiffStart + o);
      ...
      try {
        if (type === 2) {
          ...
          tags.set(
            tag,
            buf
              .slice(tiffStart + dataOffset, tiffStart + dataOffset + cnt)
              .toString("ascii")
              .replace(/\0/g, "")
              .trim(),
          );
  ...
  if (et?.den) {
    ...
    result.shutterSpeed =
      s >= 1 ? `${s.toFixed(1).replace(/\.0$/, "")}` : `1/${Math.round(1 / s)}`;
  }
