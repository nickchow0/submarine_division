...
import { type AdminPhoto, type EditState, type ReuploadPhotoUpdates } from '@/types'
import {
  updatePhoto,
...
export function usePhotoManagement(initialPhotos: AdminPhoto[]) {
  ...
  const saveEdit = useCallback(async (photo: AdminPhoto) => {
    if (!editState) return
    setSaving(true)
  ...
  const toggleVisibility = useCallback(async (id: string) => {
    ...
    try {
      ...
      // Revert on failure
      setPhotos(prev => prev.map(p => p._id === id ? { ...p, visible: photo.visible } : p))
    }
  }, [photos])
  ...
  const applyReuploadUpdates = useCallback((id: string, updates: ReuploadPhotoUpdates) => {
