...
export async function PATCH(req: Request) {
  try {
    ...
    const allowed: (keyof SiteSettings)[] = [
      'requirePassword', 'showLocations', 'maintenanceMode', 'showCaptions', 'autoGenerateCaptions',
    ]
    ...
    const existing = await sanityWriteClient.fetch<SiteSettings | null>(SITE_SETTINGS_QUERY)
    ...
    const result = await sanityWriteClient.fetch<SiteSettings | null>(SITE_SETTINGS_QUERY)
