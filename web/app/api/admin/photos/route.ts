  ...
  if (Array.isArray(node)) {
    ...
    const hasDirectRef = node.some(
      (item) =>
        item &&
        typeof item === "object" &&
        (item as Record<string, unknown>)._ref === photoId,
    );
    if (hasDirectRef) {
...
export async function PATCH(request: Request) {
  try {
    ...
    if (fields.focalLength !== undefined)
      updates.focalLength = fields.focalLength;
    if (fields.iso !== undefined) updates.iso = fields.iso;
    if (fields.shutterSpeed !== undefined)
      updates.shutterSpeed = fields.shutterSpeed;
...
export async function DELETE(request: Request) {
  try {
    const { id, imageRef } = (await request.json()) as {
      id: string;
      imageRef: string;
    };
