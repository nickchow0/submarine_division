...
function buildImageUrl(ref: string): string {
  ...
  const withoutPrefix = ref.replace(/^image-/, ""); // "Abc123-1920x1080-jpg"
  const lastDash = withoutPrefix.lastIndexOf("-");
  const nameWithDims = withoutPrefix.slice(0, lastDash); // "Abc123-1920x1080"
  const format = withoutPrefix.slice(lastDash + 1); // "jpg"
