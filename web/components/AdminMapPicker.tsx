...
export default function AdminMapPicker({
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      pins.forEach((pin) => {
        const marker = leafletL
          .marker([pin.coordinates.lat, pin.coordinates.lng], {
      ...
      if (pins.length === 1) {
        ...
        const bounds = leafletL.latLngBounds(
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      if (!pendingCoords)
        return; // Set globalThis.L before calling createPinIcon
      (globalThis as unknown as { L: typeof leafletL }).L = leafletL;

      const marker = leafletL
        .marker([pendingCoords.lat, pendingCoords.lng], {
