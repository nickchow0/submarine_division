  ...
  return (
    <>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {/* Caption filter */}
        <button
          onClick={() => setFilterCaption(filterCaption === 'all' ? 'missing' : 'all')}
        ...
        <span className="text-xs text-slate-600 ml-1">
          {photos.length}{photos.length !== allPhotos.length ? ` / ${allPhotos.length}` : ''} photo{photos.length !== 1 ? 's' : ''}
        </span>
      </div>
      ...
      {photos.length === 0 ? (
        <div className="text-center py-20 text-slate-500">
          {allPhotos.length === 0 ? 'No photos found in Sanity.' : 'No photos match the current filters.'}
        </div>
      ) : (
          ...
          {photos.map(photo => (
            ...
            >
              ...
              <div className="relative aspect-square bg-slate-950">
                ...
                {/* Selection checkbox */}
                <button
                  onClick={e => { e.stopPropagation(); onSelectId(photo._id) }}
                ...
                {feedback?.id === photo._id && (
                  <span className="absolute bottom-2 left-2 right-2 flex items-center justify-center gap-1.5 bg-sky-500/20 backdrop-blur-sm text-sky-400 text-xs px-2 py-1 rounded">
                    ...
                    {feedback.detail && onShowErrorDetail && (
                      <button
                        onClick={e => { e.stopPropagation(); onShowErrorDetail(feedback.msg, feedback.detail!) }}
              ...
              <div onClick={e => e.stopPropagation()} className="border-t border-slate-800 mt-2 px-3 py-2 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => onRegenerateCaption(photo._id, photo.imageRef)}
